...
/*
Function generate_graph takes all the relative information and generates the required graph using Chart.js

Reference ==>
https://www.chartjs.org/docs/latest/
*/
function generate_graph(time, label, points, color, type, title_x) {
  ...
  GLOBAL_GRAPH = myChart;
}

/* 
Simple function decides when to display the solved array on the GUI
*/
function show_array(decider=null) {
  if (decider === 's') {
    document.getElementById('show').hidden = true;
    document.getElementById('show_p').innerHTML = sorted_array;
    document.getElementById('unshow').hidden = false;
  }
  else {
    document.getElementById('show').hidden = false;
    document.getElementById('show_p').innerHTML = '';
    document.getElementById('unshow').hidden = true;
  }
}
